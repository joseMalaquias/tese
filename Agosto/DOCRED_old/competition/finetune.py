...
model = load_model_classic.model
tokenizer = load_model_classic.tokenizer
...
#model.to(device)
# classic
inputs = tokenizer(text=texts, entity_spans = entity_spans, task = "entity_pair_classification", return_tensors = "pt").to("cuda")
torch.save(inputs, 'inputs_classic.pt')
#inputs = torch.load("inputs_classic.pt")
...
trainer.save_model("model_finetuned_3epocs_classic")
